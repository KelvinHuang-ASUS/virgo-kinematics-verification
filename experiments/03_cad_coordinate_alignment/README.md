...
5. **Architectural Scope Division**:
   - **3D Interactive Viewer**: Focuses on physical link solid geometry and mounting location verification (rendered up to physical camera links `cam_narrow_link`, `cam_tele_link`, `cam_wide_link` matching vendor CAD Datum coordinates). Virtual algorithm optical frames (`cam_*_cv_frame`) are omitted from 3D canvas rendering to prevent spatial coordinate overlap.
   - **Kinematic TF Topology Graphs (DOT/PDF/PNG)**: Fully documents and validates the complete kinematic transformation chain including all virtual algorithm optical reference frames (`cam_*_cv_frame`) and fixed joints (`cam_*_cv_j`).
